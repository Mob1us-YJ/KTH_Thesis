...
\begin{figure}[!ht]
  ...
  \caption{Wi-Fi CSI sensing principle. (a)~A transmitter and receiver exchange signals over a line-of-sight (LoS) path and multiple reflected paths; a human body in the environment introduces an additional dynamic multipath component $\vec{H}_d$ that varies with movement. (b)~Phasor representation of the received channel: the total channel $\vec{H}$ is the vector sum of the static component $\vec{H}_s$ and the dynamic component $\vec{H}_d$; as the person moves, the tip of $\vec{H}$ traces an arc between constructive ($P_B$) and destructive ($P_A$) interference states. (c)~The corresponding CSI amplitude over time oscillates between $|\vec{H}_s|+|\vec{H}_d|$ (constructive) and $|\vec{H}_s|-|\vec{H}_d|$ (destructive), producing the characteristic amplitude fluctuation exploited by activity recognition algorithms.}
